		...
		<React.Fragment>
			<div className="highlight p-3 mb-3">
				<Row>
					<Col>
						<h4 className="m-0">
							Reviews <Badge variant="primary">{item.totalRatingCount}</Badge>
						</h4>
					</Col>
				...
				currentReview.map((review) => (
					<div className="reviews-wrapper" key={review.id}>
						<Media className="pl-3 pr-3">
							<Avatar name={review.name} size="50" round={true} className="mr-3" />
							<Media.Body>
								<Row className="mb-2 mt-2">
									<Col>
										<h5 className="m-0">{review.name}</h5>
										<small className="text-secondary">{dayjs(review.dateCreated).fromNow()}</small>
									</Col>
									<Col className="text-right">
