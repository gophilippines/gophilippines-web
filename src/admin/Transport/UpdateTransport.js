...
import { updateTransportAction, uploadTransportImageAction } from '../../redux/TransportRedux';
...
function Transport({ match }) {
	...
	useEffect(
		() => {
			async function fetchItemData() {
				const fetchItem = await fetch(`/transportationById?id=${match.params.id}`);
	...
	const updateTransport = (transport) => dispatch(updateTransportAction(transport));
	const uploadTransportImage = (formData) => dispatch(uploadTransportImageAction(formData));
					...
					<Col md={{ span: 8, offset: 2 }}>
						<Card className="mb-5">
							...
							<Card.Body
								...
								style={{
									backgroundImage: 'url(' + transport.imageURL + ')'
								}}
							>
								<h4>Update Transport Page</h4>
						...
						<Form onSubmit={onSubmit} className="mt-5">
							<Form.Group as={Row}>
								...
								<Col>
									<Form.Control company="name" onChange={onChange} value={transport.company} />
								</Col>
							</Form.Group>
							<Form.Group as={Row}>
								<Form.Label column sm="3">
